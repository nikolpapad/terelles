...
function CardPanel({ jobId }: { jobId: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 short:p-3">
        ...
        {job.funFact && (
          <Section icon="star" title="Le savais-tu ?" tone="sea">
            <div className="flex items-start gap-3">
              <p className="flex-1 text-lg short:text-base">{noEmoji(job.funFact)}</p>
              <SpeakButton text={noEmoji(job.funFact)} />
            </div>
          </Section>
        )}
      </div>
...
const TONES = {
  ...
  sea: { box: "bg-sea-light/25", title: "text-sea-deep" },
};
